...
function buildPlan(key: PlanKey, data: MarketingHubData, path: StrategyPath | null): string {
  ...
  if (key === 'tactical') {
    return [
      ...
      '',
      path === 'QUICK' ? 'ملاحظة: مسارك سريع → ركّز على الربعين ١ + ٢ فقط.' :
      path === 'MEDIUM' ? 'ملاحظة: مسارك متوسط → أنجز الأربع أرباع باستقرار.' :
      path === 'LONG' ? 'ملاحظة: مسارك طويل → استثمر في قنوات ذات نموّ مركّب (SEO + المحتوى).' :
      '',
    ].filter(Boolean).join('\n')
  }
  ...
  return [
    ...
    '• حصّة الصوت (Share of Voice)',
    '',
    path === 'LONG' ? 'ملاحظة: مسارك طويل — هذه الخطة تناسبك تماماً.' :
    path === 'MEDIUM' ? 'ملاحظة: مسارك متوسط — نفّذ السنة الأولى فقط، وأعِد التقييم.' :
    path === 'QUICK' ? 'ملاحظة: مسارك سريع — راجع فقط، لا تنفّذ بعد.' : '',
  ].filter(Boolean).join('\n')
}
