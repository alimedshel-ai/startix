...
describe('matchRiskTemplate — مطابقة ذكيّة للأسماء المستوردة', () => {
  ...
  it('«ضعف» وصفٌ لا محتوى: محاور التدقيق لا تتطابق كلها زائفاً بـ«تحصيل الذمم»', () => {
    // البلاغ: مدير FINANCE يرى ثالوث التحصيل لكل خطر تدقيق «ضعف <محور>».
    // بعد الإصلاح: لا واحد منها يطابق قالب تحصيل الذمم عبر كلمة «ضعف» وحدها.
    for (const axis of ['ضعف الحوكمة والسياسات', 'ضعف المالية والكفاءة', 'ضعف الرقمي والأتمتة']) {
      expect(matchRiskTemplate(`[تدقيق] ${axis}`, 'FINANCE')?.name).not.toBe('ضعف تحصيل الذمم المدينة')
    }
    // «ضعف الفريق والقدرات» يطابق قالبًا متعلّقًا بالفريق (relevant) لا بالتحصيل
    const team = matchRiskTemplate('[تدقيق] ضعف الفريق والقدرات', 'FINANCE')
    expect(team?.name).not.toBe('ضعف تحصيل الذمم المدينة')
    expect(team?.name).toContain('الفريق')
    // لا انحدار: خطر تحصيل حقيقيّ ما زال يطابق قالبه
    expect(matchRiskTemplate('ضعف تحصيل الذمم', 'FINANCE')?.name).toBe('ضعف تحصيل الذمم المدينة')
  })
